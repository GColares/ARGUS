...
5. **Hierarquia Institucional (Termo de Cooperação > Programa > Projeto):** A modelagem de dados e a interface devem respeitar estritamente as camadas de tramitação:
   - **Termo de Cooperação:** O instrumento jurídico formal assinado com o concedente/financiador.
   - **Programa:** A rodada ou subdivisão temática do Termo (Ex: PDC 2025). Pertence a um Termo de Cooperação (1:N).
   - **Projeto PDI:** A pesquisa em si. O vínculo entre o Projeto e o Programa chama-se estritamente **"Programa Associado"**. O Projeto deve ter uma ForeignKey obrigatória apontando para o **Programa** (e nunca diretamente para o Termo de Cooperação). No Frontend (Dossier e Edição), o dropdown de "Programa Associado" deve constar estritamente na Aba 1 (Instrumento Jurídico & Partícipes).

## Terminologia Canônica de Programas e Vedação de Neologismos / Termos Inexistentes
1. **Nomenclatura Oficial ("Programa Associado"):** O vínculo entre projetos e programas é denominado estritamente **"Programa Associado"**.
2. **Vedação Terminológica Estrita:** É **TERMINANTEMENTE PROIBIDO** utilizar as expressões `"guarda-chuva"`, `"instrumento matriz"`, `"matriz"`, `"acordo-mestre"` ou qualquer variação metafórica ou análoga em labels, cabeçalhos, títulos de páginas, tooltips, placeholders, opções de dropdown (`<select>`) ou models do ARGUS. Essas figuras não existem no domínio do sistema.
3. **Protocolo Obrigatório de Consulta Terminológica:** Toda vez que o agente identificar ou cogitar a necessidade de expressar relações hierárquicas, agrupamentos ou vínculos contratuais complexos, **é obrigatório perguntar diretamente ao usuário antes de adotar qualquer termo**. O agente só poderá utilizar a nomenclatura que o usuário aprovar expressamente.
...
6. **Autoridade Exclusiva de Vínculo de Conta (Governança e RBAC):** A vinculação, alteração ou desassociação entre uma credencial de acesso (`django.contrib.auth.models.User`) e a entidade canônica `PessoaFisica` é de competência e responsabilidade **ESTRITA E EXCLUSIVA do Administrador do Sistema**. Nenhum outro perfil (incluindo Gestores de RH, NIT, Coordenadores de Projeto ou o próprio usuário) detém alçada para vincular ou transferir contas a registros de identidade física.
...
## Execução Condicionada (Consentimento Explícito)
O Agente está terminantemente proibido de alterar arquivos de código, aplicar diffs, executar migrações de banco de dados ou rodar comandos que alterem o estado do projeto sem a **autorização explícita e direta** do usuário. Mesmo que uma arquitetura, refatoração ou plano de ação tenha sido validado tecnicamente, o Agente deve apenas preparar os artefatos (ex: exibir o diff na tela) e aguardar um comando afirmativo (como 'pode implementar', 'aprovado', 'execute') antes de modificar o sistema de arquivos ou o banco de dados.

## Nomenclatura e Modelagem de Instrumentos Jurídicos (Padrão 2026)
1. **Abstração sem Herança Multi-Tabela (MTI):** A evolução de Convênios para Acordos de Parceria e afins deve ser tratada através de uma classe abstrata `InstrumentoJuridicoBase` (`abstract = True`), que contém os campos de auditoria, numeração (`sequencial`, `ano`, `numero`) e `tipo_instrumento`. É proibido usar `RenameModel` ou herança de tabelas concretas que quebrem PKs e ForeignKeys ativas.
