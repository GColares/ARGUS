...
5. **Hierarquia de Fomento Macro (Termo > Programa > Projeto):** Projetos que fazem parte de um fomento macro não devem ser vinculados a um campo achatado. A modelagem de dados e a interface devem respeitar estritamente três camadas hierárquicas:
   ...
   - **Projeto PDI:** A pesquisa em si. Quando pertencer a um fomento macro, o Projeto deve ter uma ForeignKey obrigatória apontando para o **Programa** (e nunca diretamente para o Termo de Cooperação). No Frontend (Dossier e Edição), o dropdown de "Programa" deve constar estritamente na Aba 1 (Instrumento Jurídico & Partícipes).

## Vedação Terminológica Estrita: Proibição de Jargões Informais ("Guarda-Chuva")
É **TERMINANTEMENTE PROIBIDO** utilizar a expressão informal, leiga ou metafórica `"guarda-chuva"` (ou qualquer variação similar) em labels, cabeçalhos, títulos de páginas, tooltips, placeholders, opções de dropdown (`<select>`) ou textos da interface do ARGUS:
1. **Instrumento Matriz:** Em vez de "Termo Guarda-Chuva", utilize estritamente a designação jurídica canônica: **"Termo de Cooperação"** ou **"Acordo Mestre de Cooperação"**.
2. **Subdivisão Programática:** Em vez de "Programa Guarda-Chuva", utilize estritamente **"Programa de Fomento"**, **"Programa Temático"** ou simplesmente **"Programa"**.
3. **Modelagem e `verbose_name`:** Rótulos de campos no Django (`verbose_name`, `help_text`) e `choices` devem respeitar essa mesma sobriedade terminológica, evitando poluir formulários automáticos com termos coloquiais.
