...
2. **Geração de Backup de Segurança:**
   ...
   - Execute o dump do banco PostgreSQL EXCLUSIVAMENTE para a pasta de backups com timestamp para preservar seu estado local atual (ex: `$env:PGPASSWORD='argus'; & "C:\Program Files\PostgreSQL\18\bin\pg_dump.exe" -U postgres -d argus_db -f backups/argus_backup_YYYYMMDD_HHMM.sql`).
   - ATENÇÃO: NÃO modifique o arquivo `argus_backup.sql` que fica na raiz, pois ele é a ponte de sincronização do usuário.
